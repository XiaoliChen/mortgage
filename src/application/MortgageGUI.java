...
public class MortgageGUI{
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					MortgageGUI window = new MortgageGUI();
		...
		frame.getContentPane().add(n);
		 n.addActionListener(new java.awt.event.ActionListener(){
			...
			public void actionPerformed(ActionEvent arg0) {
				...
				double eighteen_percent = Mortgage.HousingPayment(grossIncome);
				double thirtysix = Mortgage.debtPayments(grossIncome, monthlyDebt);
		        double pmtAllowed = Mortgage.canSpend(eighteen_percent, thirtysix);
		        Double mortgage = Mortgage.mortgageAllowed(pmtAllowed, dwnPmt,r, term);
